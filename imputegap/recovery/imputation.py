...
class Imputation:
    ...
    class GPVAE(BaseImputer):
        """
        GPVAE class to impute missing values with GP-VAE.

        This integration stores:
        - self.recov_data: final imputed matrix (observed values preserved)
        - self.recov_no_gt: raw model imputation before reinserting observed values (optional)
        - self.evaluation_metrics: dict of metrics if ground_truth/y_val provided (optional)
        """
        algorithm = "gpvae"

        def impute(self, user_def=True, params=None):
            """
            params (dict | None) can include:
            - config_yaml_path (optional)
            - folder_checkpoint_path (optional)
            - epoch, batch_size, inference_batch_size
            - beta, learning_rate, sigma, length_scale, kernel_scales
            - ground_truth, y_val
            - return_no_gt_imputation, verbose, logs
            """
